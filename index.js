/* black background */
document.body.style.backgroundColor = 'black';

var count = 0
var text = document.createElement('div');
text.style.position = 'absolute';
text.style.bottom = '0px';
text.style.right = '0px';
text.style.color = 'white';
text.style.fontSize = '20px';
text.style.padding = '10px';

/* white ball */
var ball = document.createElement('div');
ball.style.width = '100px';
ball.style.height = '100px';
ball.style.backgroundColor = 'white';
ball.style.borderRadius = '50px';
ball.style.position = 'absolute';
ball.style.top = '50%';
ball.style.left = '50%';
ball.style.marginLeft = '-50px';
ball.style.marginTop = '-50px';
document.body.appendChild(ball);
/* 80% smaller */
ball.style.width = '20px';
ball.style.height = '20px';
ball.style.marginLeft = '-10px';
ball.style.marginTop = '-10px';
/* animate horizontally and vertically */
var x = Math.random() * (window.innerWidth - 20);
var y = Math.random() * (window.innerHeight - 20);
var dx = 12;
var dy = 12;
var animate = function() {
  x += dx;
  y += dy;
  ball.style.left = x + 'px';
  ball.style.top = y + 'px';
  
  text.innerHTML = '<br>count: ' + count;
  if (x < 20 && y < 20) {
    count++;
  }
  if (x > window.innerWidth - 20 && y < 20) {
    count++;
  }
  if (x < 20 && y > window.innerHeight - 20) {
    count++;
  }
  if (x > window.innerWidth - 20 && y > window.innerHeight - 20) {
    count++;
  }
  
  window.requestAnimationFrame(animate);
};
animate();
/* bounce off the walls */
var animate = function() {
  x += dx;
  y += dy;
  if (x < 0 || x > window.innerWidth - 20) {
    dx = -dx;
  }
  if (y < 0 || y > window.innerHeight - 20) {
    dy = -dy;
  }
  ball.style.left = x + 'px';
  ball.style.top = y + 'px';
 
  
  window.requestAnimationFrame(animate);
};

/* white bal */
var bal = document.createElement('div');
bal.style.width = '100px';
bal.style.height = '100px';
bal.style.backgroundColor = 'white';
bal.style.borderRadius = '50px';
bal.style.position = 'absolute';
bal.style.top = '50%';
bal.style.left = '50%';
bal.style.marginLeft = '-50px';
bal.style.marginTop = '-50px';
document.body.appendChild(bal);
/* 80% smaller */
bal.style.width = '20px';
bal.style.height = '20px';
bal.style.marginLeft = '-10px';
bal.style.marginTop = '-10px';
/* animate horizontallyb and verticallyb */
var xb = Math.random() * (window.innerWidth - 20);
var yb = Math.random() * (window.innerHeight - 20);
var dxb = 12;
var dyb = 12;
var animate = function() {
  xb += dx;
  yb += dy;
  bal.style.left = xb + 'px';
  bal.style.top = yb + 'px';
  
  text.innerHTML = '<br>count: ' + count;
  if (xb < 20 && yb < 20) {
    count++;
  }
  if (xb > window.innerWidth - 20 && yb < 20) {
    count++;
  }
  if (xb < 20 && yb > window.innerHeight - 20) {
    count++;
  }
  if (xb > window.innerWidth - 20 && yb > window.innerHeight - 20) {
    count++;
  }
  
  window.requestAnimationFrame(animate);
};
animate();
/* bounce off the walls */
var animate = function() {
  xb += dx;
  yb += dy;
  if (xb < 0 || xb > window.innerWidth - 20) {
    dxb = -dx;
  }
  if (yb < 0 || yb > window.innerHeight - 20) {
    dyb= -dy;
  }
  bal.style.left = xb + 'px';
  bal.style.top = yb + 'px';
 
  
  window.requestAnimationFrame(animate);
};
